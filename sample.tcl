# for each scenario we should set R (radio range), ST, val(x), val(y), nn, initenergy, num_row, nop, src and sink positions and sink movement
...
Agent/MyGrid instproc pr_sink_pos {xid1 yid1 xid2 yid2} {
	$self instvar node_
	puts "========================================================"
	puts "node [$node_ id] : Sink1 Position  x= $xid1 and y= $yid1"
	puts "node [$node_ id] : Sink2 Position  x= $xid2 and y= $yid2"
	puts "========================================================"
}
...
Agent/MyGrid instproc up_pos2 {xid yid ctime} {
	global ns_ p n
	$self instvar node_
	for {set i 0} {$i < $n } {incr i} {
		$ns_ at $ctime "$p($i) sink2_update_pos $xid $yid" 
	}
}
...
set opt(nn)	      531		      ;# number of mobilenodes
...
$node_(1) set X_ $fullx 
$node_(1) set Y_ $midy 
$node_(1) set Z_ 0.0 

set postable(0,0) 20.0 
set postable(1,0) $midy
set postable(0,1) $fullx 
set postable(1,1) $midy 
...
#	set src [expr int($rnd_f)]
	...
	for {set i 2} {$i < $sensor } {incr i} {	;#finding neighboring nodes at event point
		...
		if {$dton <= $R_event} {
			...
			$ns_ at $firetime "$p($i) reset_nearest_sink"
...
set dimy $midy
set ix 0
set jy [expr $num_row-1]
for {set dimx 20} {$dimx <= $fullx} {set dimx [expr $dimx + $R]} {
	set s1(0,$ix) [expr $dimx + $R]
	set s2(0,$jy) [expr $dimx + $R]
	if {$dimx < $midx} {
		set dimy [expr $dimy-$R]
		set s1(1,$ix) $dimy
		set s2(1,$jy) $dimy
	} else {
		set dimy [expr $dimy+$R]				
...
set dimy $midy
set ix [expr $num_row-1]
set jy 0
for {set dimx $fullx} {$dimx > 20} {set dimx [expr $dimx - $R]} {
	set s1(0,$ix) [expr $dimx - $R]
	set s2(0,$jy) [expr $dimx - $R]
	if {$dimx > $midx} {
		set dimy [expr $dimy + $R]
...
for {set m 0} {$m < $ST} {incr m} {
	if {$l == $m} {
        $ns_ at $l "$node_(0) setdest $s1(0,$c) $s1(1,$c) 20.0"            ;# sink1 (0) movement
        $ns_ at $l "$node_(1) setdest $s2(0,$c) $s2(1,$c) 20.0"            ;# sink2 (1) movement
	set l [ expr $l + $period ]
        set c [ expr $c + 1 ]
        if {$c == [expr ($num_row-1)*2] } { set c 0}
        }
}
...
for {set updatetime 5.5} {$updatetime < $ST } {set updatetime [ expr $updatetime + $period]} {
	...
	$ns_ at $updatetime "$p(1) updatepos"
}
...
$ns_ at 1.4 "$p(14) print_nei1"

# -----end of program------------------------
$ns_ at $ST "$p(0) print_sink_energy"
$ns_ at $ST "$p(1) print_sink_energy"
$ns_ at $ST "result"
