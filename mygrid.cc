...
/*	else if (strcmp(packet_type,"NEI") == 0) {
		...
		d1 = sqrt(((xpos-sink_x) * (xpos-sink_x))+((ypos-sink_y) * (ypos-sink_y)));
			   ...
			   if ((d2 < d1) && ((routingtable[i].residual_e) > min_e)) {
		              t=i;  
			      min_nodeid = t; 
		              min_dis = d2;
		              min_d = d2;
		...
		if ((min_nodeid >= 0) && (min_nodeid < rid)) {            
			...
			send(pkt, 0);
		        ...
		        fprintf (pfile4_, "Can not send packet-%d-%.4f-%.4f \n", source_id, CURRENT_TIME, initenergy);
			fclose(pfile4_);   
		}
...
//        iEnergy = iNode->energy_model()->energy();
        printf("updated energy for node %d is energy %.4f  \n", source_id, energy);
        //----------------------------------------------------------
        //------------- Lifetime measurment ------------------------
	if ((energy < min_e) && (sw_life == 0) && (source_id != 0) && (source_id != 1)) {
...
//        iEnergy = iNode->energy_model()->energy();
	...
	//Hello Message
        ...
        if (hdr->packet_type == 1)  {
           if (hdr->source_id != 0 && hdr->source_id != 1) {
		...
		Packet::free(pkt);
		    ...
		    tcl.eval(out);
	            ...
	            mysend(1,0,rcvd_original_id,"DATA",rcvd_x,rcvd_y,0,rcvd_sinkid,rcvd_sinkx,rcvd_sinky,rcvd_hopcount);                    	
                 }
...
void MyGridAgent::printsinkpos()
{
	...
	Tcl& tcl = Tcl::instance();
	update_position();
	sprintf(out, "%s pr_sink_pos %f %f %f %f", name(), sink1_x,sink1_y,sink2_x,sink2_y); 
	tcl.eval(out);
}
...
void MyGridAgent::update_pos()
{
	...
	if (source_id == 0) {
		...
		sprintf(out, "%s up_pos2 %f %f %f", name(), posx,posy,CURRENT_TIME);
	} 
	tcl.eval(out);
}
/*
void MyGridAgent::random_source_select()
{
        int r_n;
//        time_t seconds;
	char out[100];
...
//        srand((unsigned int) seconds);
        r_n = rand() % 26;
	sprintf(out, "%s random_source %d %f", name(), r_n, CURRENT_TIME);
	tcl.eval(out);
}
*/
void MyGridAgent::find_nearest_sink(const char* packet_type, float xpos, float ypos)
{
 //Finding the nearest sink
        if (sw==0) {
           float d1, d2;
	   d1 = sqrt(((xpos-sink1_x)*(xpos-sink1_x))+((ypos-sink1_y)*(ypos-sink1_y)));
	   d2 = sqrt(((xpos-sink2_x)*(xpos-sink2_x))+((ypos-sink2_y)*(ypos-sink2_y)));
           printf("Distance to sink1= %f and distance to sink2= %f",d1,d2);
           if (d1 < d2) {
//            nearsink_x=sink1_x;
//            nearsink_y=sink1_y;
              nearsink_id = 0;
...
//            nearsink_y=sink2_y;
        ...
        if (nearsink_id == 0) {
                mysend(0,0,0,packet_type,xpos,ypos,0,nearsink_id,sink1_x,sink1_y,0);
        } else {
                mysend(0,0,0,packet_type,xpos,ypos,0,nearsink_id,sink2_x,sink2_y,0);
        }
}
...
int MyGridAgent::command(int argc, const char*const* argv)
{
	if (argc == 2) {
		...
		else if (strcmp(argv[1], "reset_nearest_sink") == 0) {
			sw=0;
			return (TCL_OK);
		}
...
/*		else if (strcmp(argv[1], "rand_source") == 0) {
                        random_source_select();
			return (TCL_OK);
		} */
	}
...
//                        float energy = iEnergy;
		...
		if (strcmp(argv[1], "send") == 0) {
                        float x_s = atof(argv[3]);
                        float y_s = atof(argv[4]);
                        find_nearest_sink(argv[2],x_s,y_s);
			//mysend(0,0,0,argv[2],x_s,y_s,0,0,0,0);
			return (TCL_OK);
		}
		...
		else if (strcmp(argv[1], "sink2_update_pos") == 0) {
                        sink2_x = atof(argv[2]);
                        sink2_y = atof(argv[3]);
			return (TCL_OK);
		}
